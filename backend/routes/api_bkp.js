...
router.post('/claim-host', async (req, res) => {
  ...
  try {
    const [meetings] = await pool.execute(
      'SELECT creator_email FROM meetings WHERE room_id = ?',
      [meetingId]
    );
...
router.get('/meetings/:userId', async (req, res) => {
  ...
  try {
    ...
    // Use pmx_scheduled_meetings table (not meetings)
    ...
    // Merge and format results - USE THE CORRECT COLUMN NAMES
    const allMeetings = [...createdMeetings, ...invitedMeetings].map(m => ({
      id: m.id,
      roomId: m.room_id,
      title: m.meeting_title,
      host: m.creator_name,
      startTime: m.start_datetime,  // Use start_datetime column
      endTime: m.end_datetime,      // Use end_datetime column
      description: m.description,
      creatorEmail: m.creator_email,
      invitees: JSON.parse(m.invitees_json || '[]')
    }));

    console.log('Meetings fetched:', allMeetings.length);
    if (allMeetings.length > 0) {
      console.log('First meeting:', {
        id: allMeetings[0].id,
        start_datetime: allMeetings[0].startTime,
        end_datetime: allMeetings[0].endTime,
...
router.post('/schedule', async (req, res) => {
 ...
 // Create date strings in local timezone
  ...
  const emailBody = `
    ...
    <p>You have been invited to join a meeting.</p>
    <p><strong>Meeting:</strong> ${meetingTitle}</p>
    <p><strong>Date & Time:</strong> ${scheduledDate} ${startTime}</p>
    <p><strong>Creator:</strong> ${creatorName} (${creatorEmail})</p>
    <p>Click the link below to join:</p>
    <p><a href="${link}">${link}</a></p>
    <p>${description || ''}</p>
...
<body>
    <div class="email-wrapper">
        <div class="header">
            ...
            <h1 class="header-title">Meeting Invitation</h1>
            <p class="header-subtitle">You've been invited to join a virtual meeting</p>
        </div>
        ...
        <div class="content">
            ...
            <div class="button-container">
                <a href="${link}" class="join-button">🎯 Join Meeting Now</a>
