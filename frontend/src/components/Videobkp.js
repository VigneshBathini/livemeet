...
const safeJoinRoom = async () => {
  ...
  // Update the useEffect for request-join
useEffect(() => {
  if (roomId && userName && email && socketRef.current?.connected && waitingForApproval) {
    if (!isHost && !hasRequestedJoinRef.current) {
  ...
  // Reset when component unmounts or when no longer waiting
  return () => {
...
useEffect(() => {
  const autoJoin = async () => {
    if (isExternal && roomId && userName && email && validated && !inRoom) {
      ...
      if (isHost) {
        ...
        if (socketRef.current?.connected && !hasRequestedJoinRef.current) {
          hasRequestedJoinRef.current = true;
 ...
 useEffect(() => {
  console.log('9 : Setting up socket connection and listeners');
  ...
  const setupListeners = () => {
    if (socket._listenersSetup) {
      console.log('Socket listeners already set up, skipping...');
      return;
    }
    
    socket._listenersSetup = true;
    ...
    // Use a single handler for waiting-users with debounce
    let waitingUsersTimeout;
    socket.on('waiting-users', (users) => {
      console.log("Waiting users updated:", users);
      
      // Debounce to prevent multiple rapid updates
      clearTimeout(waitingUsersTimeout);
      waitingUsersTimeout = setTimeout(() => {
        setWaitingUsers(users);
      }, 100);
    });

    // Add ONE-TIME handler for host-verified
    socket.once('host-verified', (data) => {
    ...
    socket.on('host-direct-join', (data) => {
      ...
      setTimeout(async () => {
        try {
          ...
          socket.emit('join-room', roomId, socket.id, userName, email, true);
          setInRoom(true);
        } catch (err) {
    ...
    // CRITICAL FIX: Use once for join-approved to prevent multiple handlers
    socket.once('join-approved', async (data) => {
      ...
      try {
        ...
        socket.emit('join-room', roomId, socket.id, userName, email, false);
        setInRoom(true);
      } catch (err) {
    ...
    let roomUsersTimeout;
    socket.on('room-users', (users) => {
      clearTimeout(roomUsersTimeout);
      roomUsersTimeout = setTimeout(() => {
    ...
    // CRITICAL: Use regular on but check for duplicates in handlers
    socket.on('user-joined', (userId, userName, userEmail, isUserHost) => {
      handleUserJoined(userId, userName, userEmail, isUserHost);
    });
    
    socket.on('offer', handleOffer);
    socket.on('answer', handleAnswer);
    socket.on('ice-candidate', handleIceCandidate);
    socket.on('user-left', ({ userId, userName }) => handleUserLeft(userId, userName));
    ...
    let screenShareTimeout;
    socket.on('screen-share-status', (data) => {
      clearTimeout(screenShareTimeout);
      screenShareTimeout = setTimeout(() => {
  ...
  return () => {
    ...
    if (socketRef.current) {
      socketRef.current._listenersSetup = false;
...
const handleUserJoined = (userId, userName, userEmail, isUserHost) => {
  ...
  // CRITICAL: Check if we already have this peer
  if (peersRef.current[userId]) {
    logDebug(`⏭️ Skipping peer creation for ${userId}: already connected`);
    return;
  }
  
  // Check if this is a duplicate call (same user joining twice)
  if (connectionStatus[userId]) {
    logDebug(`⏭️ Skipping duplicate user-joined for ${userId}: already in connectionStatus`);
    return;
  }
